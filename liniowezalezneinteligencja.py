...
target_address = "1612PT2zpMCMRwJsaR9YYs8YPgtYCPKrYe"  # Adres do znalezienia
memory_size = 1000  # Maksymalna liczba przechowywanych wyników
learning_rate = 0.01  # Współczynnik uczenia (mniejszy dla stabilności)
max_cache_size = 1000  # Maksymalny rozmiar pamięci podręcznej
ADDRESS_FILE = "adresy.txt"  # Plik z adresami do znalezienia
OUTPUT_FILE = "znalezionoBTCOLD.TXT"  # Plik do zapisu znalezionych adresów

# === 1. Wczytywanie adresów z pliku ===
def load_addresses():
    """Wczytuje adresy Bitcoin z pliku do RAM-u (ignoruje saldo)."""
    addresses = set()

    if not os.path.exists(ADDRESS_FILE):
        print(f"❌ Błąd: Plik {ADDRESS_FILE} nie istnieje!")
        sys.exit(1)

    print("📂 Wczytywanie adresów docelowych z pliku...")
    with open(ADDRESS_FILE, "r", encoding="utf-8") as file:
        for line in file:
            parts = line.strip().split()  # Rozdzielamy adres i saldo
            if len(parts) > 0:  # Jeśli jest coś w linii
                address = parts[0].strip()  # Usuwamy dodatkowe białe znaki
                if address.startswith("1") or address.startswith("3") or address.startswith("bc1"):
                    addresses.add(address)  # Dodajemy adres Bitcoin do zbioru

    print(f"✅ Wczytano {len(addresses)} adresów do pamięci RAM.")
    return addresses

# === 2. Konwersja adresu BTC na `hash160` ===
...
# === 3. Konwersja klucza prywatnego na adres BTC ===
...
# === 4. Pobranie rzeczywistych transakcji (`r`, `s`, `z`) ===
...
# === 5. Funkcja do odzyskiwania klucza prywatnego `d` ===
def recover_d_optimized(r, s, k_candidates):
    """Odzyskuje klucz prywatny `d` z danych transakcji (r, s, z) i k_candidates."""
...
# === 6. Zapis znalezionego klucza do pliku ===
def save_found_key(d, address):
    with open(OUTPUT_FILE, "a") as file:
        file.write(f"Znaleziony adres: {address}\n")
        file.write(f"Private Key (HEX): {hex(d)}\n")
        file.write("=" * 60 + "\n")
    print(f"✅ Klucz zapisany do {OUTPUT_FILE}")

# === 7. Główna pętla ataku ===
def main():
    print("==== Rozpoczynam analizę ataków na ECDSA ====")
    print("Docelowy adres:", target_address)

    # Wczytujemy adresy z pliku
    addresses = load_addresses()

    znalezione_r = []
    raw_signatures = get_real_transactions()
    normalized_signatures = [(tx["r"], tx["s"], tx["z"]) for tx in raw_signatures]

    while True:
        for r, s, z in normalized_signatures:
            # Odzyskiwanie `d` i generowanie adresu
            d = recover_d_optimized(r, s, [random.randint(1, n) for _ in range(100)])
            if d is not None:
                addr = private_key_to_address(d)
                print(f"🎉 ZNALEZIONO KLUCZ D = {hex(d)} dla adresu: {addr}")
                
                # Porównujemy wygenerowany adres z listą docelowych adresów
                if addr in addresses:
                    print(f"🎯 ZNALEZIONO ADRES: {addr}")
                    save_found_key(d, addr)  # Zapisujemy znaleziony adres do pliku
                
                # Sprawdzamy, czy adres pasuje do docelowego
                if addr == target_address:
                    print("🎉🎉🎉 ODNALAZŁEM POSZUKIWANY ADRES! 🎉🎉🎉")
                    return

        print("Nie znaleziono klucza d, ponawiam próbę...\n")
